...
func Alignment(query_spectral []reader.Spectrum, reference_spectral, Decoy_database []reader.Spectrum, Adduct_isotope_list dataframe.Adduct_isotope, Input_param dataframe.Parameters) ([]dataframe.Grade, []reader.Spectrum) {
	...
	writer.Mappingwriter(Match_result_list, "D:/Work/XY_Meta_test/All_match_result.meta", query_spectral, reference_spectral, Adduct_isotope_list)
	...
	if query_coordinate_list.GetLength() >= mmi {
		for i := 0; i < query_coordinate_list.GetLength(); i++ {
			if query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.GetLength() != 0 {
				var sum_dot float64
				var cosine float64
				for j := 0; j < query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.GetLength(); j++ {
					sum_dot = sum_dot + query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.Search(j).(*PeakElement).weights*query_coordinate_list.Search(i).(*Coordinatelist).Headcoordinate.y*query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.Search(j).(*PeakElement).y*(1/(0.001+math.Abs(query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.Search(j).(*PeakElement).x)))
					cosine = cosine + query_coordinate_list.Search(i).(*Coordinatelist).Headcoordinate.y*query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.Search(j).(*PeakElement).y/(query_coordinate_list.Search(i).(*Coordinatelist).Headcoordinate.y*(math.Sqrt(math.Pow(query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.Search(j).(*PeakElement).x, 2)+math.Pow(query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.Search(j).(*PeakElement).y, 2))))
				}
				match_result.Dot_product = match_result.Dot_product + sum_dot/float64(query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.GetLength())
				match_result.Cosine_sum = match_result.Cosine_sum + cosine/float64(query_coordinate_list.Search(i).(*Coordinatelist).Coordinategroup.GetLength())
			} else {
				match_result.Dot_product = match_result.Dot_product + 0.0
				match_result.Cosine_sum = match_result.Cosine_sum + 0.0
			}
		}
		match_result.Cosine_similar = match_result.Cosine_sum / float64(query_coordinate_list.GetLength())
	} else {
